...
wt = "./weights/expert_resnet20_subset_" + str(args.subset) + "_kd.pth.tar"
...
def train(epoch, model, teacher, loss_fn):
    ...
    for batch_idx, (data, target) in enumerate(TrainLoaderforExpert):
        ...
        loss = loss_fn(output, target, teacher_output, T=5.0, alpha=0.8)
        loss.backward()
        optimizer.step()
...
#     train(epoch, model, teacher, loss_fn=distillation)
#     scheduler.step()
...
#fl = "./textfiles/subset" + str(args.subset) + "_result_kd.txt"
#with open(fl, 'w') as f:
...
print("The size of the Teacher Model: {}M".format(model_size/1000000))
