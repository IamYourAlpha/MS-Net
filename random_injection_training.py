...
def train(epoch, model, loss_fn):
    ...
    for batch_idx, (data, target) in enumerate(TrainLoaderforExpert):
        ...
        alpha = np.random.choice([0,1], 1).item()
        loss_kd = loss_fn(output_all, target_all, teacher_output, T=5.0, alpha=0.5)
...
for sub in SUBSET:
    ...
    for epoch in range(args.epochs):
        ...
        test(model)
    ### save the records ####
    fl = "./textfiles/rr/random_injection/subset_" + str(sub) + "_result.txt"
    with open(fl, 'w') as f:
